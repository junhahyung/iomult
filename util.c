...
char *joinpath (char *str1, char *str2)
{
    ...
    char *fullpath = tc_malloc(strlen1 + strlen2 + 2);
    memset(fullpath, 0, strlen1+strlen2+2);
...
struct hashtable *readntokenize (int fd, int docID, struct hashtable *hash_t)
{
    ...
    if (!hash_t){
        ...
        hash_t = (struct hashtable*)tc_malloc(sizeof(struct hashtable));
        memset(hash_t,0,sizeof(struct hashtable));
    }
...
tokenize (char *buf, int docID, int line, struct hashtable *hash_t)
{
    ...
    while(*temp_head)
    {
        ...
        else
        {
                ...
                term_t = (struct termstruct *)tc_malloc(sizeof(struct termstruct));
                //term_t->term = (char*)calloc(wordlen + 1, sizeof(char));
                term_t->term = (char*)tc_malloc(wordlen + 1);
                memset(term_t->term,0,wordlen+1);
                stringncat(term_t->term, temp_lag, wordlen);
                term_t->total_freq = 1;
                term_t->next_pos = (struct pos*)tc_malloc(sizeof(struct pos));
...
void
search_v2 (struct hashtable* hash_t, char* targetword, char** docDic, char* out)
{
    int hashn = 0;
    int rb = 0;
    int wordlen, targetlen, linenum, docID;
    ...
    while(cur_t)
    {
        ...
        if((wordlen == targetlen) && (!stringncmp(cur_t->term, targetword, wordlen)))
        {
            ...
            while(cur_pos)
            {
                ...
                docID = cur_pos->position[0];
                linenum = cur_pos->position[1];
                sprintf(line, "%s: line #%d\n", docDic[docID], linenum);
				int linelen = strlen(line);
				strncpy(out+rb, line, strlen(line)+1);
                cur_pos = cur_pos->next_pos;
				rb += strlen(line);
            }
            break;
